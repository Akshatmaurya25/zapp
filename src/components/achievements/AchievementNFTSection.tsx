...
export default function AchievementNFTSection({ userId, isOwnProfile }: AchievementNFTSectionProps) {
  ...
  }, [fetchEligibleAchievements, fetchMintedNFTs, fetchAchievementProgress, fetchSimpleAchievements])

  const fetchEligibleAchievements = useCallback(async () => {
    try {
      const response = await fetch(`/api/achievements/eligible?userId=${userId}`)
  ...
  const handleMintNFT = async (achievementId: string) => {
    ...
    try {
      ...
      if (ethereum.providers) {
        ...
        ethereum = ethereum.providers.find((provider: any) => provider.isMetaMask) || ethereum
      }
      ...
      success('Transaction Sent!', 'NFT minting transaction sent to blockchain')
      ...
      if (receipt.status === 1) {
        ...
        try {
          ...
          const transferLog = receipt.logs.find((log: any) => log.topics[0] === '0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef')
        ...
        success('NFT Minted Successfully!', `Token ID: ${tokenId} | TX: ${tx.hash.slice(0, 10)}...`)
        ...
        try {
          ...
          console.warn('Failed to update database (NFT still minted successfully):', dbError)
        }
      ...
      if (err.code === 4001) {
        ...
        error('Minting Error', err.message || 'An unexpected error occurred during minting')
      }
    } finally {
      setMintingId(null)
    }
  ...
  const handleCreateAchievements = async () => {
    ...
    try {
      ...
      if (response.ok && data.achievementsCreated?.length > 0) {
        success('Achievements Created!', `Created ${data.achievementsCreated.length} achievements: ${data.achievementsCreated.join(', ')}`)
        await fetchAchievementData()
        return
      }
      ...
      if (response.ok && data.achievementsFound > 0) {
        success('NFT Achievements Created!', `Found ${data.achievementsFound} achievements for minting`)
        await fetchAchievementData()
      } else {
        error('All Methods Failed', 'Could not create achievements. Check console for details.')
  ...
  const getRarityColor = (rarity_level: number) => {
    switch (rarity_level) {
      case 1: return 'border-gray-500 bg-gray-500/10 text-gray-400'
      case 2: return 'border-green-500 bg-green-500/10 text-green-400'
      case 3: return 'border-blue-500 bg-blue-500/10 text-blue-400'
      case 4: return 'border-purple-500 bg-purple-500/10 text-purple-400'
      case 5: return 'border-yellow-500 bg-yellow-500/10 text-yellow-400'
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'journey': return <Star className="h-5 w-5" />
      case 'creator': return <Award className="h-5 w-5" />
      case 'social': return <Trophy className="h-5 w-5" />
      case 'loyalty': return <Medal className="h-5 w-5" />
      case 'gaming': return <Zap className="h-5 w-5" />
    ...
    <div className="space-y-8">
        ...
        <div className="bg-gradient-to-r from-yellow-500/10 to-orange-500/10 border border-yellow-500/30 rounded-xl p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-yellow-200 mb-1">Demo Mode Active</h3>
              <p className="text-sm text-yellow-300/80 mb-3">
                NFT minting is currently in demo mode. NFTs are simulated and not stored on the blockchain.
              </p>
              <div className="text-xs text-yellow-400/70 space-y-1">
                <p>🔧 To enable real blockchain minting:</p>
                <p>1. Run: <code className="bg-black/30 px-2 py-0.5 rounded">npm run setup:nft</code></p>
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-text-secondary max-w-2xl mx-auto">
          ...
          Showcase your platform journey through blockchain-verified achievements.
        </p>
      </div>
        ...
        <Card className="border-purple-500/20 bg-purple-500/5">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-purple-400">
              ...
              Your Achievements Ready to Mint ({simpleAchievements.length} available)
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      {isOwnProfile && eligibleAchievements.length === 0 && achievementProgress.some(p => p.progress_percent === 100) && (
          ...
          <CardContent>
            <div className="text-center space-y-4">
              <p className="text-text-secondary">
                You have completed achievements that can be converted to mintable NFTs!
              </p>
              <Button
        ...
        <Card className="border-yellow-500/20 bg-yellow-500/5">
          ...
          <CardContent>
            <div className="space-y-2 text-sm">
              <p>Eligible Achievements (NFT System): {eligibleAchievements.length}</p>
              ...
              <p>100% Completed: {achievementProgress.filter(p => p.progress_percent === 100).length}</p>
              <div className="mt-4 space-y-2">
                <Button
                  onClick={async () => {
                    console.log('Testing user data access...')
                    try {
                      const response = await fetch(`/api/achievements/direct?userId=${userId}`)
                      const data = await response.json()
                      console.log('Direct achievements response:', data)
                      if (data.achievements?.length > 0) {
                        success('Found Achievements!', `${data.achievements.length} achievements available`)
                        setSimpleAchievements(data.achievements)
                      } else {
                        error('No Achievements', 'No achievements found or generated')
                      }
                    } catch (err) {
                ...
                <details>
                  <summary className="cursor-pointer text-blue-400 text-xs">Show Achievement Data</summary>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => window.open(`https://shannon-explorer.somnia.network/tx/${nft.tx_hash}`, '_blank')}
              className="text-xs"
            >
...
function AchievementProgress({
  ...
  const categories = ['all', 'journey', 'creator', 'social', 'loyalty', 'gaming', 'special']

  const filteredProgress = selectedCategory === 'all'
    ? progress
    : progress.filter(p => p.category === selectedCategory)

  const inProgressAchievements = filteredProgress.filter(p => !p.is_earned && p.progress_percent > 0)
          ...
          <div className="space-y-4">
            {inProgressAchievements.slice(0, 6).map((item) => (
              <div key={item.id} className="space-y-2">
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg ${getRarityColor(item.rarity_level).split(' ')[1]}`}>
