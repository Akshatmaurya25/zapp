...
contract Custody is IChannel, IDeposit, IChannelReader, EIP712 {
    ...
    function create(Channel calldata ch, State calldata initial) public returns (bytes32 channelId) {
        ...
        _copyState(meta.lastValidState, initial);
        ...
        for (uint256 i = 0; i < PART_NUM; i++) {
            ...
            meta.actualDeposits[i] = Amount({token: address(0), amount: 0});
        }
    ...
    function join(bytes32 channelId, uint256 index, bytes calldata sig) external returns (bytes32) {
        ...
        _copyState(meta.lastValidState, lastValidState);
        meta.stage = ChannelStatus.ACTIVE;
    ...
    function close(bytes32 channelId, State calldata candidate, State[] calldata) public {
        ...
        if (meta.stage == ChannelStatus.ACTIVE) {
            ...
            _copyState(meta.lastValidState, candidate);
    ...
    /**
     * @notice Helper function to copy State struct from memory to storage
     * @param target Storage State to copy to
     * @param source Memory State to copy from
     */
    function _copyState(State storage target, State memory source) internal {
        target.intent = source.intent;
        target.version = source.version;
        target.data = source.data;

        // Copy allocations array
        delete target.allocations;
        for (uint256 i = 0; i < source.allocations.length; i++) {
            target.allocations.push(source.allocations[i]);
        }

        // Copy signatures array
        delete target.sigs;
        for (uint256 i = 0; i < source.sigs.length; i++) {
            target.sigs.push(source.sigs[i]);
        }
    }
    ...
    function _unlockAllocation(bytes32 channelId, Allocation memory alloc) internal {
        ...
        meta.tokenBalances[alloc.token] = channelBalance - correctedAmount;
